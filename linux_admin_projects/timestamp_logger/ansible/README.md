...
This folder contains an optional Ansible extension for the timestamp-logger project. It automates the installation and uninstallation of the service on a Raspberry Pi (or any Linux system).
...
```
Copying the log_time.sh script to /usr/local/bin
...
```
ansible/
├── playbook.yml               # Main playbook to install timestamp-logger
├── uninstall.yml              # Optional playbook to uninstall
└── files/
    ├── log_time.sh            # Service script used by playbook
    └── systemd/
    ...
    files/log_time.sh: Source script to be installed
...
```
journalctl -u timestamp-logger.service -f
```
